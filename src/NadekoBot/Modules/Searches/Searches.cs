    ...
    public partial class Searches : NadekoTopLevelModule<SearchesService>
    {
        ...
        public async Task Weather([Remainder] string query)
        {
            ...
            var embed = new EmbedBuilder()
                .AddField(fb => fb.WithName("🌍 " + Format.Bold(GetText("location"))).WithValue($"[{data.Name + ", " + data.Syas.Country}](https://openweathermap.org/city/{data.Id})").WithIsInline(true))
                ...
                .AddField(fb => fb.WithName("🔆 " + Format.Bold(GetText("min_max"))).WithValue($"{data.Main.TempMin}°C - {data.Main.TempMax}°C, {dataf.Main.TempMin}°F - {dataf.Main.TempMax}°F").WithIsInline(true))
